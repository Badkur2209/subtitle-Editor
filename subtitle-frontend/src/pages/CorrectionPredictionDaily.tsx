...
export default function PredictionsDaily() {
  ...
  const languageSetList = useMemo(() => {
    ...
    activities.forEach((activity) => {
      ...
      const primaryCol = targetLangKey;
    ...
    return list;
  }, [activities, targetLangKey, filterDate, filterLagna]);

  const handleSelectLanguageSet = (item) => {
    setSelectedLanguageSet(item);

    const sourceCols = LANGUAGE_COLUMNS_MAP[targetLangKey];
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-bold mb-4">Correction - Prediction</h2>
          ...
          <div className="w-2/3">
            {selectedLanguageSet ? (
              <div>
                ...
                // CHANGE from grid-cols-2 to single column: // Your current
                code has a formatting issue, fix it to:
                {/* Dynamic Translation Rows */}
                <div className="space-y-4">
                  ...
                  {LANGUAGE_COLUMNS_MAP[targetLangKey].map((colKey, index) => {
              ...
              <div className="text-center text-gray-500 py-20">
                <p>Select a language set from the list to start Corecting</p>
              </div>
            )}
          </div>
        </div>
