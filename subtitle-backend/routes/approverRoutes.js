...
// router.all("/activity", async (req, res) => {
//   try {
//     // Pick language from either query or body
//     const { language = "en" } = req.body.language ? req.body : req.query;

//     const statusColumn = `status_${language}`;
//     const actColumn = `act_${language}`;

//     const items = await ActivityData.findAll({
//       where: { [statusColumn]: "inreview" },
//       attributes: [
//         "id",
//         "fromdate",
//         "todate",
...
// routes/approverRoutes.js
router.post("/activity/status", async (req, res) => {
  const { id, status, language } = req.body;
  ...
  try {
    ...
    await ActivityData.update(updateData, {
      where: { id },
    });
...
router.post("/predictiondaily/status", async (req, res) => {
  ...
  try {
    const item = await PredictionDaily.findByPk(id);
    if (!item) {
      return res.status(404).json({ error: "Item not found" });
    }
...
router.get("/predictiondaily", async (req, res) => {
  try {
    const { language = "en" } = req.query;
    const statusColumn = `status_${language}`;

    const items = await PredictionDaily.findAll({
      where: {
        [statusColumn]: "inreview",
      },
      order: [["fromdate", "DESC"]],
